...
local Utility = {}
local Objects = {}
function Manticore:DraggingEnabled(frame, parent)
...
local Name = "ManticoreConfig.JSON"
...
local CoreName = tostring(math.random(1, 100))..tostring(math.random(1,50))..tostring(math.random(1, 100))

function Manticore:ToggleUI()
    if game.CoreGui[CoreName].Enabled then
        game.CoreGui[CoreName].Enabled = false
    else
        game.CoreGui[CoreName].Enabled = true
    end
end

function Manticore.CreateLib(ManName, themeList)
    if not themeList then
        themeList = themes
    end
    if themeList == "Manticore" then
        themeList = themeStyles.Manticore
    end

    themeList = themeList or {}
    local selectedTab 
    ManName = ManName or "Manticore"
    table.insert(Manticore, ManName)
    for i,v in pairs(game.CoreGui:GetChildren()) do
        if v:IsA("ScreenGui") and v.Name == ManName then
            v:Destroy()
        end
    end
    ...
    Manticore:DraggingEnabled(MainHeader, Main)
    ...
    function Manticore:ChangeColor(prope,color)
